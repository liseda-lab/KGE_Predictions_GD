# -*- coding: utf-8 -*-
'''
    File name: Performance_ML_10fold.py
...
def create_X_and_Y_lists(file_operation_path, file_indexes_path):
    ...
    print('....................... X AND Y TRAIN/TEST CREATED.......................')
    index_file.close()
...
def performance_NayveBayes(X_train, X_test, y_train, y_test, path_output_predictions):
    ...
    predictions_test = gnb.predict(np.array(X_test))
    predictions_train = gnb.predict(np.array(X_train))
    predictions_prob_test = gnb.predict_proba(np.array(X_test))
    predictions_prob_train = gnb.predict_proba(np.array(X_train))
...
def performance_MLP(X_train, X_test, y_train, y_test, path_output_predictions):
    ...
    clf = GridSearchCV(mlp_model,
                       {'hidden_layer_sizes': [(50, 50, 50), (50, 100, 50), (100,)],
                        'activation': ['tanh', 'relu'],
                        'solver': ['sgd', 'adam'],
                        'alpha': [0.0001, 0.05],
                        'learning_rate': ['constant', 'adaptive']}, n_jobs=-1)
...
##              Run Performance             ##
##############################################
operations_list = [('Data_Concatenated.csv', 'Concatenation'), ('Data_Average.csv', 'Average'), ('Data_Hadamard.csv', 'Hadamard'),
...
for operation in operations_list:  # Embedding Operation
    for i in range(1, 11):  # Partition
        Xtrain, Ytrain = create_X_and_Y_lists(operation[0], '_10Fold_indexes/10Fold_indexes__crossvalidationTrain_Run_' + str(i) + '.txt')
        Xtest, Ytest = create_X_and_Y_lists(operation[0], '_10Fold_indexes/10Fold_indexes__crossvalidationTest_Run_' + str(i) + '.txt')

        path_output_predictions = 'RandomForest_Run' + str(i) + '_' + str(operation[1])
...
for operation in operations_list:  # Embedding Operation
    for i in range(1, 11):  # Partition
        ...
        path_output_predictions = 'XGboost_Run' + str(i) + '_' + str(operation[1])
...
for operation in operations_list:  # Embedding Operation
    for i in range(1, 11):  # Partition
        ...
        path_output_predictions = 'NayvesBayes_Run_' + str(i) + '_' + str(operation[1])

        waf, fmeasure_noninteract, fmeasure_interact, precision, recall, \
            accuracy, auc = performance_NayveBayes(Xtrain, Xtest, Ytrain, Ytest, path_output_predictions)
...
for operation in operations_list:  # Embedding Operation
    for i in range(1, 11):  # Partition
        ...
        path_output_predictions = 'MLP_Run_' + str(i) + '_' + str(operation[1])

        waf, fmeasure_noninteract, fmeasure_interact, \
            precision, recall, accuracy, auc, parameters = performance_MLP(Xtrain, Xtest, Ytrain, Ytest, path_output_predictions)
